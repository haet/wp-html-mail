<div class="postbox">
	<h3 class="hndle"><span><?php esc_html_e( 'Advanced features', 'wp-html-mail' ); ?></span></h3>
	<div style="" class="inside">
		<table class="form-table">
			<tbody>

				<tr valign="top">
					<th scope="row"><label><?php esc_html_e( 'Import / Export template', 'wp-html-mail' ); ?></label></th>
					<td>
						<a href="#" class="button-secondary haet-mail-toggle-export-button"><?php esc_html_e( 'Export template', 'wp-html-mail' ); ?></a>
						<a href="#" class="button-secondary haet-mail-toggle-import-button"><?php esc_html_e( 'Import template', 'wp-html-mail' ); ?></a>
						<div class="haet-mail-toggle-export">
							<textarea readonly><?php echo esc_textarea( str_replace( '\\&quot;', '', wp_json_encode( $theme_options ) ) ); ?></textarea>
							<p class="description">
								<?php esc_html_e( 'Copy the settings above and paste into another site.', 'wp-html-mail' ); ?>
							</p>
						</div>
						...
						<div class="haet-mail-toggle-import">
							...
							<p class="description">
								<?php esc_html_e( 'Paste settings from another site to the field above.', 'wp-html-mail' ); ?>
							</p>
							<a href="#" class="button-secondary haet-mail-import-start"><?php esc_html_e( 'Start Import', 'wp-html-mail' ); ?></a>
						</div>
					</td>
				</tr>

				<tr valign="top">
					<th scope="row"><label><?php esc_html_e( 'Content type', 'wp-html-mail' ); ?></label></th>
					<td>
						...
						<label for="haet_mail_invalid_contenttype_to_html"><?php esc_html_e( 'Allow HTML code in plain text content type', 'wp-html-mail' ); ?></label>
						<p class="description">
							<?php esc_html_e( 'From security perspective you should not enable this option but some plugin developers send HTML code in their emails without setting the correct content type header, so you might see HTML tags in your emails.', 'wp-html-mail' ); ?>
						</p>
					</td>
				</tr>
				<tr valign="top">
					<th scope="row"><label><?php esc_html_e( 'Delete plugin settings', 'wp-html-mail' ); ?></label></th>
					<td>
						<a href="<?php echo esc_url( add_query_arg( 'advanced-action', 'delete-design' ) ); ?>" class="button-secondary" data-haet-confirm="<?php esc_attr_e( 'Are you sure? This can not be undone!', 'wp-html-mail' ); ?>"><?php esc_html_e( 'Delete design settings', 'wp-html-mail' ); ?></a>
						<a href="<?php echo esc_url( add_query_arg( 'advanced-action', 'delete-all' ) ); ?>" class="button-secondary" data-haet-confirm="<?php esc_attr_e( 'Are you sure? This can not be undone!', 'wp-html-mail' ); ?>"><?php esc_html_e( 'Delete ALL settings', 'wp-html-mail' ); ?></a>
						<?php do_action( 'haet_mail_plugin_reset_buttons' ); ?>
					</td>
				</tr>
				<?php if ( class_exists( 'WPHTMLMail_Woocommerce' ) ) : ?>
					<tr valign="top">
						<th scope="row"><label><?php esc_html_e( 'Mailbuilder custom posts', 'wp-html-mail' ); ?></label></th>
						<td>
							<a href="<?php echo esc_url( get_admin_url( null, 'edit.php?post_type=wphtmlmail_mail' ) ); ?>" class="button-secondary"><?php esc_html_e( 'Show all emails', 'wp-html-mail' ); ?></a>
							<p class="description">
								<?php esc_html_e( 'All emails created with our mailbuilder are stored as custom post types. You can manage them at the link above', 'wp-html-mail' ); ?>
							</p>
						</td>
					</tr>
				<?php endif; ?>
				...
				?>
				<tr valign="top">
					<th scope="row"><label><?php esc_html_e( 'Create custom template', 'wp-html-mail' ); ?></label></th>
					<td>
						<input type="hidden" name="haet_mail_create_template">
						<button type="button" id="haet_mail_create_template_button" class="button-secondary <?php echo ( $theme_is_writable ? '' : 'button-disabled' ); ?>" data-haet-confirm="<?php esc_attr_e( 'Are you sure you want to write your own code?', 'wp-html-mail' ); ?>"><?php esc_html_e( 'create template file in my theme folder', 'wp-html-mail' ); ?></button>
						<?php if ( file_exists( get_stylesheet_directory() . '/wp-html-mail/template.html' ) ) : ?>
							<p><?php esc_html_e( 'You already have a custom template. If you create a new one the existing template will be backed up.', 'wp-html-mail' ); ?></p>
						<?php endif; ?>
						<?php if ( ! $theme_is_writable ) : ?>
							<p><?php esc_html_e( 'WARNING: Your theme directory is not writable by the server. Please change the permission to allow us to create the mail template.', 'wp-html-mail' ); ?></p>
						<?php endif; ?>
						<p class="description">
						...
						<div id="haet_mail_template_created" class="haet-mail-dialog" title="<?php esc_html_e( 'Template created', 'wp-html-mail' ); ?>">
							<p>
								<?php esc_html_e( 'Your template has been created.', 'wp-html-mail' ); ?>
							</p>
						</div>
					</td>
				</tr>
