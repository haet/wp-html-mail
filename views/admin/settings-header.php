...
?>
<div class="postbox">
	<h3 class="hndle"><span><?php esc_html_e( 'Header', 'wp-html-mail' ); ?></span></h3>
	<div style="" class="inside">
		<table class="form-table">
			<tbody>
				<tr valign="top">
					<th scope="row"><label for="haet_mailheaderimg_placement"><?php esc_html_e( 'Image and text placement', 'wp-html-mail' ); ?></label></th>
					<td>
						<?php
						...
						<select id="haet_mailheaderimg_placement" name="haet_mail_theme[headerimg_placement]">
							<?php foreach ( $placement_options as $key => $label ) : ?>
								<option value="<?php echo esc_attr( $key ); ?>" <?php echo ( $theme_options['headerimg_placement'] == $key ? 'selected' : '' ); ?>>
				...
				<tr valign="top">
					<th scope="row"><label for="haet_mailheaderbackground"><?php esc_html_e( 'Background color', 'wp-html-mail' ); ?></label></th>
					<td>
						<input type="text" class="color" id="haet_mailheaderbackground" name="haet_mail_theme[headerbackground]" value="<?php echo esc_attr( $theme_options['headerbackground'] ); ?>">
					</td>
				</tr>
				<tr class="collapse-headerimg">
					<th scope="row">
						<label for="haet_mailheaderimg"><?php esc_html_e( 'Header image', 'wp-html-mail' ); ?></label>
					</th>
					<td>
						<div class="uploader">
							<?php
							...
							?>
							<input id="haet_mailheaderimg" name="haet_mail_theme[<?php echo esc_attr( $headerimg_field_key ); ?>]" type="text" value="<?php echo esc_attr( $headerimg ); ?>"/>
							<input id="haet_mailfilepicker" class="button upload_image_button" name="haet_mailfilepicker" type="text" value="<?php esc_html_e( 'Select image', 'wp-html-mail' ); ?>" />
							...
							<div class="collapse-headerimg_advanced">
								<label for="haet_mailheaderimg_width"><?php esc_html_e( 'width', 'wp-html-mail' ); ?></label>
								<input id="haet_mailheaderimg_width" name="haet_mail_theme[headerimg_width]" type="number" value="<?php echo esc_attr( $theme_options['headerimg_width'] ); ?>"/>px
								<label for="haet_mailheaderimg_height"><?php esc_html_e( 'height', 'wp-html-mail' ); ?></label>
								<input id="haet_mailheaderimg_height" name="haet_mail_theme[headerimg_height]" type="number" value="<?php echo esc_attr( $theme_options['headerimg_height'] ); ?>"/>px
								...
								<span class="headerimg-align">
									<?php esc_html_e( 'align', 'wp-html-mail' ); ?>
							...
							<label for="haet_mail_theme_headerimg_enable_translation">
								<?php esc_html_e( 'Enable translation for this field', 'wp-html-mail' ); ?> 
						...
						<?php if ( isset( $theme_options['headerimg_notice'] ) && $theme_options['headerimg_notice'] && stripos( $headerimg, 'templates.wp-html-mail.com' ) ) : ?>
							<p class="description"><?php echo esc_attr( $theme_options['headerimg_notice'] ); ?></p>
						<?php endif; ?>
					</td>
				</tr>
				<tr>
					<th scope="row">
						<label for="haet_mailheaderpadding"><?php esc_html_e( 'Padding', 'wp-html-mail' ); ?></label>
					</th>
					<td>
						<?php esc_html_e( 'top', 'wp-html-mail' ); ?>:
						<select  id="haet_mailheaderpaddingtop" name="haet_mail_theme[headerpaddingtop]">
							<?php for ( $padding = 0; $padding <= 50; $padding++ ) : ?>
								<option value="<?php echo esc_attr( $padding ); ?>" <?php echo ( $theme_options['headerpaddingtop'] == $padding ? 'selected' : '' ); ?>><?php echo esc_attr( $padding ); ?></option>        
							<?php endfor; ?>
						</select>
						&nbsp;&nbsp; 
						<?php esc_html_e( 'right', 'wp-html-mail' ); ?>:
						<select  id="haet_mailheaderpaddingright" name="haet_mail_theme[headerpaddingright]">
							<?php for ( $padding = 0; $padding <= 50; $padding++ ) : ?>
								<option value="<?php echo esc_attr( $padding ); ?>" <?php echo ( $theme_options['headerpaddingright'] == $padding ? 'selected' : '' ); ?>><?php echo esc_attr( $padding ); ?></option>      
							<?php endfor; ?>
						</select>
						&nbsp;&nbsp; 
						<?php esc_html_e( 'bottom', 'wp-html-mail' ); ?>:
						<select  id="haet_mailheaderpaddingbottom" name="haet_mail_theme[headerpaddingbottom]">
							<?php for ( $padding = 0; $padding <= 50; $padding++ ) : ?>
								<option value="<?php echo esc_attr( $padding ); ?>" <?php echo ( $theme_options['headerpaddingbottom'] == $padding ? 'selected' : '' ); ?>><?php echo esc_attr( $padding ); ?></option>     
							<?php endfor; ?>
						</select>
						&nbsp;&nbsp; 
						<?php esc_html_e( 'left', 'wp-html-mail' ); ?>:
						<select  id="haet_mailheaderpaddingleft" name="haet_mail_theme[headerpaddingleft]">
							<?php for ( $padding = 0; $padding <= 50; $padding++ ) : ?>
								<option value="<?php echo esc_attr( $padding ); ?>" <?php echo ( $theme_options['headerpaddingleft'] == $padding ? 'selected' : '' ); ?>><?php echo esc_attr( $padding ); ?></option>       
							<?php endfor; ?>
						</select>
					</td>
				</tr>
				<tr>
					<th scope="row">
						<label for="haet_mailheadertext"><?php esc_html_e( 'Header text', 'wp-html-mail' ); ?> <?php $this->multilanguage->maybe_print_language_label( $theme_options, 'footer' ); ?></label>
					</th>
					<td>
						<?php
						...
						?>
						<input type="text" value="<?php echo esc_attr( $headertext ); ?>" id="haet_mailheadertext" name="haet_mail_theme[<?php echo esc_attr( $headertext_field_key ); ?>]">     
						<?php if ( $this->multilanguage->is_multilanguage_site() ) : ?>
				...
				<tr>
					<th scope="row">
						<label for="haet_mailheaderfont"><?php esc_html_e( 'Font', 'wp-html-mail' ); ?></label>
					</th>
					<td>
						<?php
							Haet_Mail()->font_toolbar(
								array(
									'font'     => array(
